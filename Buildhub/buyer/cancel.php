...
/* 1) Verify the order belongs to this buyer and can be cancelled */
$check = $conn->prepare("SELECT id, status, product_name, schedule_date FROM orders WHERE id = ? AND buyer_id = ? LIMIT 1");
...
$update = $conn->prepare("UPDATE orders SET status = 'cancelled' WHERE id = ? AND buyer_id = ?");
