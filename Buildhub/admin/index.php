...
if ($conn && $sellerId > 0) {
  ...
  $sql = "SELECT COUNT(*) as count FROM orders WHERE supplier_id = ? AND schedule_date BETWEEN ? AND ?";
  if ($stmt = mysqli_prepare($conn, $sql)) {
    mysqli_stmt_bind_param($stmt, 'iss', $sellerId, $weekStart, $weekEnd);
  ...
  $monthEnd = date('Y-m-t');
  $sql = "SELECT COUNT(*) as count FROM orders WHERE supplier_id = ? AND status = 'completed' AND schedule_date BETWEEN ? AND ?";
  if ($stmt = mysqli_prepare($conn, $sql)) {
    mysqli_stmt_bind_param($stmt, 'iss', $sellerId, $monthStart, $monthEnd);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    $row = $result->fetch_assoc();
  ...
  // Total orders
  $sql = "SELECT COUNT(*) as count FROM orders WHERE supplier_id = ?";
  if ($stmt = mysqli_prepare($conn, $sql)) {
  ...
  // Pending orders
  $sql = "SELECT COUNT(*) as count FROM orders WHERE supplier_id = ? AND status = 'pending'";
  if ($stmt = mysqli_prepare($conn, $sql)) {
    mysqli_stmt_bind_param($stmt, 'i', $sellerId);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
  ...
  $sql = "SELECT SUM(total_amount) as total FROM orders WHERE supplier_id = ? AND status = 'completed'";
  if ($stmt = mysqli_prepare($conn, $sql)) {
    mysqli_stmt_bind_param($stmt, 'i', $sellerId);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
  ...
  if ($stmt = mysqli_prepare($conn, $sql)) {
    ...
    $result = mysqli_stmt_get_result($stmt);
    $row = $result->fetch_assoc();
    $stats['products_listed'] = $row['count'] ?? 0;
    mysqli_stmt_close($stmt);
  }
}
...
$recentOrders = [];
if ($conn && $sellerId > 0) {
  $sql = "SELECT o.id, o.product_name, o.status, o.schedule_date, o.total_amount, o.ordered_at, o.recipient_name, o.phone
          FROM orders o
          WHERE o.supplier_id = ? 
...
if ($conn && $sellerId > 0) {
  $sql = "SELECT DAY(schedule_date) as day, COUNT(*) as count, GROUP_CONCAT(product_name) as products
          FROM orders 
          WHERE supplier_id = ? AND DATE_FORMAT(schedule_date, '%Y-%m') = ?
          GROUP BY DAY(schedule_date)";
  if ($stmt = mysqli_prepare($conn, $sql)) {
    mysqli_stmt_bind_param($stmt, 'is', $sellerId, $currentMonth);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    while ($row = $result->fetch_assoc()) {
...
<head>
    ...
    <title>BuildHub Seller Dashboard</title>
    ...
    <div class="flex h-screen bg-gray-50">
        ...
        <div class="flex-1 ml-16 md:ml-64 p-6 overflow-auto">
            ...
            <div class="mb-6">
                ...
                <p class="text-gray-600 mt-2">Manage your products, orders, and deliveries from one place.</p>
            </div>

            <!-- Stats Cards -->
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4 sm:gap-6 mb-6">
    <!-- Deliveries This Week -->
    <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 shadow-sm flex items-center">
        ...
        <div class="ml-3 sm:ml-4">
            <h3 class="text-xs sm:text-sm font-medium text-gray-500">Deliveries This Week</h3>
            <p class="text-lg sm:text-2xl font-bold text-gray-900"><?php echo h($stats['deliveries_this_week']); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 shadow-sm flex items-center">
        ...
        <div class="ml-3 sm:ml-4">
            <h3 class="text-xs sm:text-sm font-medium text-gray-500">Completed This Month</h3>
            <p class="text-lg sm:text-2xl font-bold text-gray-900"><?php echo h($stats['completed_this_month']); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 shadow-sm flex items-center">
        ...
        <div class="ml-3 sm:ml-4">
            <h3 class="text-xs sm:text-sm font-medium text-gray-500">Total Orders</h3>
            <p class="text-lg sm:text-2xl font-bold text-gray-900"><?php echo h($stats['total_orders']); ?></p>
        </div>
    </div>

    <!-- Pending Orders -->
    <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 shadow-sm flex items-center">
        <div class="p-2 sm:p-3 bg-orange-100 rounded-lg flex-shrink-0">
        ...
        <div class="ml-3 sm:ml-4">
            <h3 class="text-xs sm:text-sm font-medium text-gray-500">Pending Orders</h3>
            <p class="text-lg sm:text-2xl font-bold text-gray-900"><?php echo h($stats['pending_orders']); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 shadow-sm flex items-center">
        ...
        <div class="ml-3 sm:ml-4">
            <h3 class="text-xs sm:text-sm font-medium text-gray-500">Total Revenue</h3>
            <p class="text-lg sm:text-2xl font-bold text-gray-900"><?php echo h(peso($stats['total_revenue'])); ?></p>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 shadow-sm flex items-center">
        ...
        <div class="ml-3 sm:ml-4">
            <h3 class="text-xs sm:text-sm font-medium text-gray-500">Products Listed</h3>
            <p class="text-lg sm:text-2xl font-bold text-gray-900"><?php echo h($stats['products_listed']); ?></p>
        </div>
    </div>
</div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                    <div class="flex items-center space-x-2 mb-6">
                        ...
                        <h2 class="text-xl font-semibold text-gray-700">Delivery Schedule</h2>
                    </div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <a href="myorders.php" class="text-sm text-blue-600 hover:text-blue-800 font-medium">Manage All</a>
                    </div>
                    ...
                    <div class="space-y-3">
                        <?php if (empty($recentOrders)): ?>
                            <div class="text-center py-8">
                                ...
                                <a href="orders.php" class="text-blue-600 hover:text-blue-800 text-sm font-medium">Add Products</a>
                            </div>
                        <?php else: ?>
                            <?php foreach ($recentOrders as $order): ?>
                                ...
                                <div class="flex justify-between items-center p-4 bg-gray-50 border border-gray-200 rounded-lg hover:shadow-sm transition-shadow cursor-pointer" 
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm text-gray-500">Customer: <?php echo h($order['recipient_name']); ?></p>
                                        <p class="text-sm text-gray-500">Scheduled: <?php echo h(fmtDate($order['schedule_date'])); ?></p>
                                        <p class="text-sm text-gray-500">Phone: <?php echo h($order['phone']); ?></p>
                                    </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <a href="orders.php" class="flex items-center p-4 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors">
                        ...
                        <div>
                            <h3 class="font-medium text-blue-900">Add Products</h3>
                            <p class="text-sm text-blue-600">List new construction materials</p>
                        </div>
                    </a>
                    ...
                    <a href="myorders.php" class="flex items-center p-4 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition-colors">
                        ...
                        <div>
                            <h3 class="font-medium text-green-900">Manage Orders</h3>
                            <p class="text-sm text-green-600">Accept, complete, and track orders</p>
                        </div>
                    </a>
                    ...
                    <a href="settings.php" class="flex items-center p-4 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-colors">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-purple-600">Manage your seller account</p>
                        </div>
                    </a>

                    <a href="orders.php" class="flex items-center p-4 bg-orange-50 border border-orange-200 rounded-lg hover:bg-orange-100 transition-colors">
                        ...
                        <div>
                            <h3 class="font-medium text-orange-900">Update Products</h3>
                            <p class="text-sm text-orange-600">Edit existing product listings</p>
                        </div>
                    </a>
                </div>
            </div>
    ...
    <script>
        function showDayDetails(day, orderCount) {
            ...
            if (orderCount > 0) {
                details.innerHTML = `
                    <p class="text-gray-600">You have <strong>${orderCount}</strong> deliver${orderCount > 1 ? 'ies' : 'y'} scheduled for ${day}.</p>
                    <div class="mt-4">
